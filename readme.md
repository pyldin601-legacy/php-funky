...
Option::isEmpty();
Option::nonEmpty();
Option::getOrElse($else);
Option::getOrThrow($exceptionClass, ...$arguments);
Option::orElse(Option $other);
Option::map($callable); 
Option::filter($callable);
Option::wrap($value, $reject = null);
```
